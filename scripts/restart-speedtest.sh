...
export SPEEDTEST_INTERVAL_SECONDS="${SPEEDTEST_INTERVAL_SECONDS:-300}"
export SPEEDTEST_DURATION_SECONDS="${SPEEDTEST_DURATION_SECONDS:-30}"
export SPEEDTEST_ADDITIONAL_ARGS="${SPEEDTEST_ADDITIONAL_ARGS:--P 4}"
export SPEEDTEST_EXPORTER_PORT="${SPEEDTEST_EXPORTER_PORT:-9820}"

nohup node "$ROOT_DIR/scripts/iperf-exporter.mjs" \
  > "$OUT_FILE" \
  2>&1 &
